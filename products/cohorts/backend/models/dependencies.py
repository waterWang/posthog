...
def cohort_behavioral_shape_changed_supersede(sender, instance, **kwargs):
    ...
    A run pins the cohort's filters and shape hashes (see backfill/runs.py), and an active run blocks
    a new one, so a stale run left behind after an edit would also lock the operator out of re-running
    the backfill. `_leaf_shape_changed` is only set for allowlisted realtime, non-static, non-deleted
    cohorts on a real `filters` change, so it is the whole guard.
    """
