...
def _create_loop_task_and_run(loop: Loop, trigger: LoopTrigger | None, trigger_context: str) -> tuple[Task, TaskRun]:
    ...
    if slack_thread_context is not None and loop.created_by_id is not None:
        # A thread-bound run is a Slack-interaction run: `interaction_origin` is what gates the
        # pending-reply relay that posts the final message into the thread, and the stamped actor
        # (the loop owner) is who thread follow-ups and credential minting resolve against, the
        # same as for mention-created runs.
